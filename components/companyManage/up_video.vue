<template>
  <div>
    <input type="file" :style="m_style" id="upload" value="多文件上传" multiple :accept="mime" @click="getUpload()"
           :disabled="!upload_type"/>
      ...
      <div class="cover-con">
        ...
        <ul>
          <li v-for="(item,index) in upload_item" :key="index">
            <div class="icon">
              <img src="/company_manage/images/class/icon-video.png" alt="">
            </div>
            <!--名称、大小-->
  ...
  export default {
    ...
    methods: {
      ...
      get_voucher: function (title, filename, filesize, cateid) {
        ...
        var key_info;
        jQuery.ajax({
          url: _this.$store.state.company.baseUrl + '/common/video/create_upload',
          type: 'POST',
          dataType: 'json',
          headers: {
            token: _this.$store.state.company.token,
          },
          async: false,
          data: {
      ...
      update_voucher: function (videoId) {
        const _this = this;
        jQuery.ajax({
          url: _this.$store.state.company.baseUrl + '/common/video/refresh_upload',
          type: 'POST',
          dataType: 'json',
          headers: {
            token: _this.$store.state.company.token,
      ...
      create_uploader: function (uploadAuth, uploadAddress, videoId, index, size, t) {
        ...
        var uploader = new AliyunUpload.Vod({
          ...
          'onUploadstarted': function (uploadInfo) {
            ...
            _this.$emit('video_list_type_change', true);
            console.log('开始上传了......');
            _this.upload_type = false;
          ...
          'onUploadSucceed':async function (uploadInfo) {
            ...
            _this.upload_vedio_nums--;
            var name;
            var nameArr = uploadInfo.file.name.split('.');
            if(nameArr.length>1) {
              nameArr.pop()
              name = nameArr.join('.');
            }
            ...
            let paramst = {url: "/company/company_video/video_add",data: {upload_name: name,video_id: videoId,size: size,duration: 0,video_type: type,}}
            let rest  = await _this.$store.dispatch("companyHttpPost",paramst);

            // jQuery.ajax({
            //   url: _this.$store.state.company.baseUrl + '/company/company_video/video_add',
            //   type: 'POST',
            //   dataType: 'json',
            //   headers: {
            //     token: _this.$store.state.company.token,
          ...
          'onUploadEnd': function (uploadInfo) {
            let num = 1;
            _this.upload_item.forEach(function (val, key) {
              if (val.state === 3) {
                num = 0
              } else {
                num++
      ...
      update_upload_item: function () {
        var _this = this;
        for (let i = 0; i < _this.upload_control_item.length; i++) {
      ...
      getUpload() {
        ...
        var key_info = '';
        var upload_array;
        console.log(_this.upload_type);
        if (_this.upload_type) {
            ...
            document.getElementById("upload").addEventListener('change', function (event) {
              ...
              for (let i = 0; i < event.target.files.length; i++) {
                ...
                if (z === 0) {
                  ...
                  //获取上传凭证
                  key_info = _this.get_voucher(event.target.files[i].name, event.target.files[i].name, event.target.files[i].size, 768542560);
                  //创建uploader对象
    ...
    watch: {
      ...
      upload_type: function () {
        this.$emit('upload_type_change', this.upload_type);
      }
    },
    head() {
      return {
