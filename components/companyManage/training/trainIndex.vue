...
<div class="app_preview">
   ...
   <el-form :model="addInfo" :rules="rules" ref="addInfo" >
      ...
      <el-form-item label="*训练营封面（PC）" :label-width="formLabelWidth"  prop="training_cover_pc" v-if="!isOwn">
 
              <img v-if="addInfo.training_cover_pc" :src="addInfo.training_cover_pc.value" class="avatar" style="width:282px;height: 163px;">
 
      </el-form-item>

      <el-form-item label="训练营原价" :label-width="formLabelWidth"  prop="cost_price" v-if="!isOwn">
        {{addInfo.cost_price}}元
      </el-form-item>
      <el-form-item label="训练营现价" :label-width="formLabelWidth"  prop="pay_price" v-if="!isOwn">
        {{addInfo.pay_price}}元
      </el-form-item>
 
      <el-form-item label="上架状态" :label-width="formLabelWidth" prop="status" v-if="!isOwn">
     ...
     <div class="base_tab">
      ...
      <div :class="!isStage?'tab hover':'tab'" v-on:click="setIsStage(false)" v-if="!isOwn">训练营介绍</div>
     </div> 

      <div class="el-dialog-header" v-if="!isOwn && !isStage"><span class="el-dialog-title"></span>训练营简介 </div>
      <div  class="ql-editor" v-if="!isOwn && !isStage">{{addInfo.training_description}}</div>

      <div class="el-dialog-header" v-if="!isOwn && !isStage"><span class="el-dialog-title"></span>详情介绍 (APP) </div>
      <div  class="ql-editor" v-html="addInfo.content_app" v-if="!isOwn && !isStage"></div>

      <div class="el-dialog-header" v-if="!isOwn && !isStage"><span class="el-dialog-title"></span>详情介绍 (PC) </div>
      <div  class="ql-editor" v-html="addInfo.content_pc" v-if="!isOwn && !isStage"></div>
      ...
      <div  class="training_more" v-if="isStage">
        ...
        <div class="stage_content">
          ...
          <div v-for="(it,index) in addInfo.stage_content" :key="index" class="stage_content_son" v-if="addInfo.stage_count>index">
               ...
               <el-form-item :label=" !isOwn?'关卡积分奖励':'经验值奖励'" 
               :label-width="formLabelWidth"  
               :prop="'stage_content['+index+'].stage_score'" 
               :rules="{required: true, message: '必填', trigger:'blur' }">
                 {{!isOwn?it.stage_score:it.stage_jingyan}}
               </el-form-item>
...
export default {
	name: "caseEdit",
  props: ['addInfo','classIds','imgs','sections','editInfo','stage_map','stage_count','stage_pic','stagePicInfo','isOwn'], 
  components: {
      cropper
  },
	data() {
