     ...
     <div class="table-responsive">
       <table class="table">
        ...
        <tr v-for="(vo, index) in updateInfo" :key="index"  >
          ...
          <td>{{vo.person.birthday?vo.person.birthday:'--'}}</td>
          <td>{{vo.created_at}}</td>
          <td>
...
<el-dialog title="提示" append-to-body :visible.sync="dialogSVisible" width="30%">
   <p style="text-align: center; margin-top: 20px">{{isOne?'待该用户同意您的邀请后，加入您的企业':'待用户同意您的邀请后，加入您的企业'}} </p>
   <div slot="footer" class="dialog-footer" style="text-align:center">
    <el-button type="primary" @click="addSubmit('addInfo')" v-if="isOne">发送邀请</el-button>
    <el-button type="primary" @click="addSubmitAll('addInfo2')" v-else>发送邀请</el-button>
  </div>
</el-dialog>

 
<el-dialog title="设置初始密码" append-to-body :visible.sync="dialogSetPWDVisible" width="30%">
    <p style="text-align: center; margin-top: 20px">该用户尚未注册，请为他设置一个6~16位的初始密码 </p>
   ...
   <div slot="footer" class="dialog-footer" style="text-align:center">
    ...
    <el-button type="primary" @click="dialogSVisible = true" v-else>确 定</el-button>
  </div>
</el-dialog>
    ...
    <div class="table-responsive" v-if="isCourse">
       <table class="table">
        ...
        <tr v-for="(vo, index) in personInfo.course" :key="index"  >
          ...
          <td>{{vo.get_type==5?vo.companycourse.teacher:vo.course.teacher.realname}}</td>
          <td>{{vo.get_type==5?vo.companycourse.created_at:vo.course.created_at}}</td>
          <td>{{(Number(vo.get_type==5?vo.companycourse.all_time:vo.course.all_time)/60).toFixed(2)}}</td>
          ...
          if (valid) {
            if(this.saveAction=='edit') {
              ...
              if(res.code==0) {
                 ...
                 this.addInfo.password = "xiangxue123"
