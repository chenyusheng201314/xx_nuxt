<template>
  ...
  <div>
    <p class="return_back"><span @click="back(true)"><label>&#xe642;</label>返回</span></p>
    <!-- 个人信息-->
    <div class="infor_list">
      ...
      <div class="infor-right">
        <h3>{{user_info.person.mobile}}</h3>
        <p>
          <span class="span1">昵称：{{user_info.person.username}}</span>
          <span class="span2">联系地址：{{user_info.person.address}}</span>
          <span class="span3">所在公司：{{user_info.person.company_name}}</span>

        </p>
        <p>
          <span class="span1">生日：{{user_info.person.birthday}}</span>
          <span class="span2">最近登录：{{user_info.person.last_login_time}}</span>
          <span class="span3">公司所在行业：{{user_info.person.company_industry}}</span>
        </p>
        <p>
          <span class="span1">性别：{{user_info.person.sex === 0? '保密' : user_info.person.sex === 1? '男' : '女'}}</span>
          <span class="span2">注册时间：{{user_info.person.register_time}}</span>
          <span class="span3">现任职位：{{user_info.person.company_position}}</span>
        </p>
        <p>
          <span class="span1">身份证：{{user_info.person.identification_number}}</span>
          <span class="span2">微信：{{user_info.person.wechat === '' || user_info.person.wechat == null ? '未绑定' : user_info.person.wechat}}</span>
          <span class="span3">从事培训年限：{{user_info.person.years_of_training}}</span>
        </p>
        <p>
          <span class="span1">邮箱：{{user_info.person.email===''||  user_info.person.email == null? '未绑定' : user_info.person.email}}</span>
          <span class="span2">QQ：{{user_info.person.qq=== '' ||  user_info.person.qq == null? '未绑定' : user_info.person.qq}}</span>
      ...
      <div class="cover-con">
          ...
          <div v-show="detail_show === 2">
            <ul class="detail-list">
              <li v-for="(item,index) in pt_list" :key="index">
                <span :class="{sure : tab_change === 1, error: tab_change === 2 }">{{tab_change === 2 ? '-' : '+' }}{{item.pay_money}}</span>
        ...
        <div class="about_lesson" v-show="about_lesson_show">
          <p class="tab"
            ...
            <span :class="{active : table_show === 1}" @click="table_tab_change(1)">已购课程</span>
            <span :class="{active : table_show === 2}" @click="table_tab_change(2)">被赠送课程</span>
            <span :class="{active : table_show === 3}" @click="table_tab_change(3)">授权课程</span>
            <span :class="{active : table_show === 4}" @click="table_tab_change(4)">企业课程</span>
            <span :class="{active : table_show === 6}" @click="table_tab_change(6)">学完的课程</span>
          </p>
          <p class="tab" v-show="table_show === 11 || table_show === 10">
            <span :class="{active : table_show === 11}" @click="table_tab_change(11)">获赠记录</span>
            <span :class="{active : table_show === 10}" @click="table_tab_change(10)">赠送记录</span>
          </p>
          <p class="tab" v-show="table_show === 7|| table_show === 8 || table_show === 9">
            <span :class="{active : table_show === 7}" @click="table_tab_change(7)">已购买训练营</span>
            <span :class="{active : table_show === 8}" @click="table_tab_change(8)">授权训练营</span>
            <span :class="{active : table_show === 9}" @click="table_tab_change(9)">企业训练营</span>
          </p>
          <p class="tab" v-show="table_show === 12 || table_show ===13 || table_show === 14">
            <span :class="{active : table_show === 12}" @click="table_tab_change(12)">关注课程</span>
            <span :class="{active : table_show === 13}" @click="table_tab_change(13)">关注导师</span>
            <span :class="{active : table_show === 14}" @click="table_tab_change(14)">关注训练营</span>
          </p>
          <!-- 课程-->
