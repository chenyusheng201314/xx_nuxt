  ...
  export default {
    ...
    methods: {
      ...
      setMeau() {
        ...
        //用户列表
        let person = this.comsys.adminGetMeau(roles, "person")
        let person_list = []
        if (person.length > 0) {
        ...
        if (person_list.length > 0) {
          leftNav.push({name: '用户列表', link: '/admin/user', active: false})
        }

        //授权列表
        let sq = this.comsys.adminGetMeau(roles, "person_authorization_record")
        let sq_list = []
        if (sq.length > 0) {
          sq_list = this.comsys.adminGetMeau(sq[0].son, "course_auth_log_list")
        }
        if (sq_list.length > 0) {
          leftNav.push({name: '授权记录', link: '/admin/user/accreditLits', active: false})
        }
    ...
    async created() {
      ...
      const that = this;
      this.see_user_id = this.personId;
      this.list_show = false;
      let params = {
        url: '/manage/person/show',
        data: {
      ...
      console.log('用户信息', that.user_info);
    },
  }
</script>
