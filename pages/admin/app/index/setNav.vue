   ...
   <div class="app_index_setting">
     ...
     <div class="banner_info" v-for="(item, index) in sortNavInfo" :key="index">
         ...
         <div class="banner_sort">
           ...
           <img src="/admin/images/app_index_down.png" v-on:click="setSortDown(index)" v-if="indexInfo.navInfo.length!=(index+1)" /> 
         </div>
         </div>
    ...
    <div class="app_index_setting_btn">
        ...
        <span v-on:click='submit()'>保存</span>
         ...
         <el-dialog title="选择导师" :visible.sync="dialogTchVisible" width="70%">
          <selectTch 
          v-on:editSuccess="editSuccess"/>
      </el-dialog>
      <el-dialog title="选择训练营" :visible.sync="dialogTrainingVisible" width="70%">
          <selectTraining 
          v-on:editSuccess="editSuccess"/>
      </el-dialog>
      <el-dialog title="选择课程" :visible.sync="dialogClassVisible" width="70%">
          <selectClass 
    ...
    getTypeVal(banner,event){
      ...
      if(banner.link_type == 11 || banner.link_type == 12 ) {
        ...
        this.$set(banner,'info','全部课程列表')
         ...
         banner.link = ""
         let obj = {};
         obj = this.type[3].options.find((item)=>{    
            return item.value === banner.link_type;//比如：选项2
         });
         this.$set(banner,'info',obj.label)
