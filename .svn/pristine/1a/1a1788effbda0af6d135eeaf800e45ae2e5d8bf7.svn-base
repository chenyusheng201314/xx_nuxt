    ...
    <div class="promotion" v-loading.fullscreen.lock="fullscreenLoading">
      ...
      <table>
        ...
        <tr v-for="(item,index) in lesson_lists" :key="index">
          ...
          <td width="120px">
            <input type="text" v-model="item.points" class="now-price" @focus="get_old_price(item.points)" @blur="price_change(item.points,item.id)">
          </td>
          <td width="150px">
            <!--精品-->
    ...
    methods: {
      ...
      detele_one(ind, item) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          axios({
            method: 'post',
            url: '/manage/course_activity/course_jf_del',
            baseURL: that.$store.state.admin.baseUrl,
            headers: {
              token: that.$store.state.admin.token,
            },
