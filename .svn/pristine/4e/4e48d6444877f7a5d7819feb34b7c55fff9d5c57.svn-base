        ...
        <div class="stage_content">
          <div class="stage_set">
            ...
            <el-form-item label="选择地图" :label-width="formLabelWidth"  prop="map_num">
               <el-select v-model="addInfo.map_num"   placeholder="请选择" @change="setMapNum" disabled>
                  <el-option
            ...
            <el-form-item label="训练营关卡数" :label-width="formLabelWidth"   prop="stage_count">
               <el-select v-model="addInfo.stage_count" :value="1" placeholder="请选择" @change="setStage" disabled >
                  <el-option
          ...
          <div v-for="(it,index) in addInfo.stage_content" :key="index" class="stage_content_son" v-if="index<addInfo.stage_count">
               ...
               <el-select v-model="it.stage_pct"   placeholder="请选择">
                ...
                <b>关卡课程</b><div v-on:click="addClass(index)" class="base_btn_add"  v-if='false'> <strong>+</strong> 增加</div>
...
export default {
  ...
  methods: {
    ...
    async update() {
      console.log(this.del_section_ids)
      ...
      for(var i in this.addInfoCopy.stage_content) {
        ...
        delete this.addInfoCopy.stage_content[i].stage_score
      ...
      let params = {url: "/company/training/training_edit_save",data:this.addInfoCopy}
      ...
      if(res.code==0) {
        ...
        if(this.del_section_ids.length>0) {
          ...
          let params = {url: "/company/training/training_section_del",data:{section_ids:this.del_section_ids.join(',')}}
          let res  = await this.$store.dispatch("companyHttpPost",params);
      
        }
        if(this.del_training_ids.length>0) {
          let params = {url: "/company/training/training_stage_del",data:{stage_ids:this.del_training_ids.join(',')}}
          let res  = await this.$store.dispatch("companyHttpPost",params);
        }
      }
      else {
    ...
    cancel:function(){
     ...
     async delClass(index,ind) {
       // if(this.delClassIndex[index].length==0) {
       //    this.$message.error('请先选择课程')
       //    return false
       // }

        let checkInfo = this.comsys.adminCheckRole(this.$store.state.company.companyRole.data,'training','training_section_del')
        if(!checkInfo) {
  ...
  watch: {
    classIds:async function (val,oldVal) {
      ...
      for (var  index in this.newClassIds){
           ...
           this.addInfo.stage_content[this.stageIndex].sections.push({section_id:-1,course_id:arrs[0],course_type:arrs[1]})
      ...
      //if(!this.sections[this.stageIndex]) {this.sections[this.stageIndex] = []}
