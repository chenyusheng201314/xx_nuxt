    ...
    <div class="promotion" v-loading.fullscreen.lock="fullscreenLoading">
      ...
      <table>
        ...
        <tr v-for="(item,index) in lesson_lists" :key="index">
          ...
          <td width="300px">
            <p class="name2">{{item.course.course_name}}&nbsp;{{item.time_free.time}}</p>
          </td>
          <td width="120px">{{item.course.teacher}}</td>
          <td width="100px">{{item.course.price}}</td>
          <td width="220px">
            <el-date-picker
              v-model="item.time_free.time"
              type="datetime"
              placeholder="选择日期时间"
              format="yyyy-MM-dd HH:mm:ss"
              value-format="yyyy-MM-dd HH:mm:ss"
              default-time="00:00:00"
              @focus="get_time(item.time_free.time)"
              @blur="change_time(item.time_free.time,item.id)"
            >
            </el-date-picker>
          </td>
          <td width="150px">
...
<script>
  ...
  export default {
    ...
    async asyncData({store}) {
      // 获取课程列表
      let list_data = {
        url: '/manage/course_activity/course_free_list',
        data: {
          page: 1,
          psize: 10,
          first_sort_id: '',
    ...
    methods: {
      ...
      detele_one(ind, item) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          axios({
            method: 'post',
            url: '/manage/course_activity/course_free_del',
      ...
      up(ind, id) {
        ...
        // 接口
        axios({
          url: '/manage/course_activity/course_free_up',
          baseURL: that.$store.state.admin.baseUrl,
          method: 'post',
          headers: {
            token: that.$store.state.admin.token,
      ...
      down(ind, id) {
        ...
        // 接口
        axios({
          url: '/manage/course_activity/course_free_down',
          baseURL: that.$store.state.admin.baseUrl,
          method: 'post',
          headers: {
            token: that.$store.state.admin.token,
      ...
      change_time(time,id){
        const that = this;
        if(that.now_time !== time){
          axios({
            method: 'post',
            url: '/manage/course_activity/course_free_change_time',
            baseURL: that.$store.state.admin.baseUrl,
            headers: {
              token: that.$store.state.admin.token,
            },
      ...
      sure_add() {
        ...
        if (that.already_choose.join(',') === '') {
          ...
          axios({
            method: 'post',
            url: '/manage/course_activity/course_free_add',
            baseURL: that.$store.state.admin.baseUrl,
            headers: {
              token: that.$store.state.admin.token
            },
