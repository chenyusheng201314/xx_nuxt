    ...
    <div class="promotion" v-loading.fullscreen.lock="fullscreenLoading">
      ...
      <table>
        ...
        <tr v-for="(item,index) in lesson_lists" :key="index">
          ...
          <td width="300px">
            <p class="name2">{{item.course.course_name}}</p>
          </td>
          <td width="120px">{{item.course.teacher}}</td>
          <td width="100px">{{item.course.price}}</td>
          <td width="120px">
            <input type="text" v-model="item.sale_price" class="now-price" @blur="price_change(item.sale_price,item.id)"
                   @focus="get_old_price(item.sale_price)">
          </td>
          <td width="100px">{{(parseInt(item.sale_price) * 10 / parseInt(item.course.price)).toFixed(1)}}</td>
          <td width="150px">
            <!--精品-->
          ...
          <td width="120px"><span class="detele-one" @click="detele_one(index)">删除</span></td>
          <td class="last-td">
        ...
        <div class="cover-con">
          ...
          <table>
            ...
            <tr v-for="(item,index) in choose_lists" :key="index">
              <td width="80px">
                <img src="/admin/images/promotion/no-choose.png" alt="" v-show="!item.isChoose && item.is_activity"
                     class="img-choose"
                     @click="choose_one(index,true,item.id)">

                <img src="/admin/images/promotion/choose.png" alt="" v-show="item.isChoose && item.is_activity"
                     class="img-choose"
                     @click="choose_one(index,false,item.id)">
                <img src="/admin/images/promotion/disable-choose.png" alt="" class="img-choose"
                     v-show="!item.is_activity">
              </td>
              ...
              <td width="150px">{{item.organization_teacher.realname}}</td>
...
<script>
  ...
  export default {
    ...
    async asyncData({store}) {
      // 获取课程列表
      let list_data = {
        url: '/manage/course_activity/course_tj_list',
        data: {
          page: 1,
          psize: 10,
          first_sort_id: '',
    ...
    methods: {
      ...
      async get_video_list() {
        ...
        video_res.data.data.forEach(function (vv, kk) {
          if (vv.isChoose && vv.is_activity) {
            i += 0
          } else if (!vv.isChoose && vv.is_activity) {
            i += 1
          }
        });
        // 全选
      ...
      // 价格变化
      price_change(price, id) {
        const that = this;
        if (that.now_price !== price) {
          axios({
            method: 'post',
            url: '/manage/course_activity/course_tj_change_price',
      ...
      detele_one(ind) {
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          axios({
            method: 'post',
            url: '/manage/course_activity/course_tj_del',
            baseURL: that.$store.state.admin.baseUrl,
            headers: {
              token: that.$store.state.admin.token,
            },
      ...
      up(ind,id) {
        const that = this;
        this.lesson_lists.splice(ind, 1, ...that.lesson_lists.splice(ind - 1, 1, that.lesson_lists[ind]));
        // 接口
        axios({
          url: '/manage/course_activity/course_tj_up',
          baseURL: that.$store.state.admin.baseUrl,
          method: 'post',
          headers: {
            token: that.$store.state.admin.token,
      ...
      down(ind,id) {
        ...
        // 接口
        axios({
          url: '/manage/course_activity/course_tj_down',
          baseURL: that.$store.state.admin.baseUrl,
          method: 'post',
          headers: {
            token: that.$store.state.admin.token,
      ...
      choose_one(ind, bool, id) {
        ...
        $.each(that.choose_lists, function (key, val) {
          if (!val.isChoose && val.is_activity) {
            i++
          }
        });
        if (i === 0) {
      ...
      sure_add() {
        ...
        if (that.already_choose.join(',') === '') {
          ...
          axios({
            method: 'post',
            url: '/manage/course_activity/course_tj_add',
            baseURL: that.$store.state.admin.baseUrl,
            headers: {
              token: that.$store.state.admin.token
            },
